    ...
    defined(ENABLE_RGB_MATRIX_TYPING_HEATMAP)
    ...
    for (uint8_t i_col = 0; i_col < MATRIX_COLS; i_col++) {
      ...
      if (i_row == row && i_col == col) {
        g_rgb_frame_buffer[row][col] =
            qadd8(g_rgb_frame_buffer[row][col],
                  RGB_MATRIX_TYPING_HEATMAP_INCREASE_STEP);
      } else {
#define LED_DISTANCE(led_a, led_b)                                       \
  sqrt16(((int16_t)(led_a.x - led_b.x) * (int16_t)(led_a.x - led_b.x)) + \
         ((int16_t)(led_a.y - led_b.y) * (int16_t)(led_a.y - led_b.y)))
        uint8_t distance = LED_DISTANCE(
            g_led_config.point[g_led_config.matrix_co[row][col]],
...
bool TYPING_HEATMAP(effect_params_t* params) {
  ...
  if (params->iter == 0) {
    decrease_heatmap_values = timer_elapsed(heatmap_decrease_timer) >=
  ...
  uint8_t count = 0;
  for (uint8_t row = 0;
       row < MATRIX_ROWS && count < RGB_MATRIX_LED_PROCESS_LIMIT; row++) {
    for (uint8_t col = 0; col < MATRIX_COLS && RGB_MATRIX_LED_PROCESS_LIMIT;
         col++) {
      if (g_led_config.matrix_co[row][col] >= led_min &&
        ...
        HSV hsv = {
            170 - qsub8(val, 85), rgb_matrix_config.hsv.s,
            scale8((qadd8(170, val) - 170) * 3, rgb_matrix_config.hsv.v)};
        RGB rgb = rgb_matrix_hsv_to_rgb(hsv);
        rgb_matrix_set_color(g_led_config.matrix_co[row][col], rgb.r, rgb.g,
                             rgb.b);
