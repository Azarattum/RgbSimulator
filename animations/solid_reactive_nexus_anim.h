...
static HSV SOLID_REACTIVE_NEXUS_math(HSV hsv, int16_t dx, int16_t dy,
                                     uint8_t dist, uint16_t tick) {
  ...
  hsv.h =
      scale16by8(g_rgb_timer, qadd8(rgb_matrix_config.speed, 8) >> 4) + dy / 4;
#else
...
bool SOLID_REACTIVE_NEXUS(effect_params_t* params) {
  return effect_runner_reactive_splash(qsub8(g_last_hit_tracker.count, 1),
                                       params, &SOLID_REACTIVE_NEXUS_math);
}
#endif
...
#endif  // !defined(ENABLE_RGB_MATRIX_SOLID_REACTIVE_NEXUS) ||
        // !defined(ENABLE_RGB_MATRIX_SOLID_REACTIVE_MULTINEXUS)
