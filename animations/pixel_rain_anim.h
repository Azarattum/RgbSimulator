...
static inline uint32_t interval(void) {
  return 500 / scale16by8(qadd8(rgb_matrix_config.speed, 16), 16);
}

static void rain_pixel(uint8_t led_index, effect_params_t* params) {
  if (!HAS_ANY_FLAGS(g_led_config.flags[led_index], params->flags)) {
    return;
  }
  HSV hsv = (random8() & 2) ? (HSV){0, 0, 0}
                            : (HSV){random8(), random8_min_max(127, 255),
...
static bool PIXEL_RAIN(effect_params_t* params) {
  ...
  if (g_rgb_timer > wait_timer) {
    rain_pixel(random8_max(RGB_MATRIX_LED_COUNT), params);
  }
