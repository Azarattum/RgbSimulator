...
bool DIGITAL_RAIN(effect_params_t* params) {
  ...
  const uint8_t pure_green_intensity =
      (((uint16_t)rgb_matrix_config.hsv.v) * 3) >> 2;
  const uint8_t max_brightness_boost =
      (((uint16_t)rgb_matrix_config.hsv.v) * 3) >> 2;
  ...
  for (uint8_t col = 0; col < MATRIX_COLS; col++) {
    for (uint8_t row = 0; row < MATRIX_ROWS; row++) {
      ...
      } else if (g_rgb_frame_buffer[row][col] > 0 &&
                 g_rgb_frame_buffer[row][col] < max_intensity) {
      ...
      if (led_count > 0) {
        if (g_rgb_frame_buffer[row][col] > pure_green_intensity) {
          ...
          const uint8_t green =
              (uint8_t)((uint16_t)max_intensity * g_rgb_frame_buffer[row][col] /
    ...
    for (uint8_t row = MATRIX_ROWS - 1; row > 0; row--) {
      for (uint8_t col = 0; col < MATRIX_COLS; col++) {
        ...
        if (g_rgb_frame_buffer[row - 1][col] >=
            max_intensity) {  // Note: can be larger than max_intensity if val
                              // was recently decreased
